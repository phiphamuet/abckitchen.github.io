...
router.get('/list',admin_not_logged_in,function(req,res){
    var sql='SELECT user.username as username,user.fullname as fullname, user.birthdate as birthdate,ISNULL(thanh_toan.ngay) as paid , sum(mon_an.don_gia) as amount from dang_ky_an left join thuc_don_hang_ngay on dang_ky_an.ngay=thuc_don_hang_ngay.ngay left join mon_an on (thuc_don_hang_ngay.ma_mon=mon_an.ma_mon) left join user on user.id=dang_ky_an.user_id left join thanh_toan on (thanh_toan.user_id=user.id and month(thanh_toan.ngay)=month(thuc_don_hang_ngay.ngay) and year(thanh_toan.ngay)=year(thuc_don_hang_ngay.ngay) ) ' +
